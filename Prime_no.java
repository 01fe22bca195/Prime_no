...
public class Prime_no {
    ...
    // then stored in the variable `start`.
       ...
       // The line `scanner.close();` is closing the `Scanner` object. This is done to release any system
       // resources associated with the `Scanner` object and to prevent any potential resource leaks. It is
    ...
    private static void PrimesInRange(int start, int end) {
        ...
        // The code `System.out.println();` is used to print a new line to the console. It is used in
        // this program to print a blank line after printing all the prime numbers in the given range.
        System.out.println(); 
    }

    // The `isPrime` method is a helper method that takes in an integer `num` as a parameter and
    ...
    private static boolean isPrime(int num) {
       ...
       // The line `return true;` is used to indicate that the given number is prime. It is reached
       // when the for loop in the `isPrime` method does not find any factors of the number `num`. In
       // other words, if the number `num` is not divisible by any number from 2 to the square root of
